      ...
      steps: [
        ...
        },
        {
          run: {
            ...
            command: `$(npm bin)/cypress run --spec cypress/integration/Tests/${value} --reporter mochawesome --reporter-options "reportFilename=test${index + 1}"`,
          },
        },
...
const files = fs.readdirSync(path.join(__dirname, '..', 'cypress', 'integration','Tests')).filter(fn => fn.endsWith('.spec.js'));
// read circle.json
